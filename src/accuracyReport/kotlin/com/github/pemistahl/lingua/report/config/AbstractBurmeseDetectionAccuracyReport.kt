...
abstract class AbstractBurmeseDetectionAccuracyReport(
    implementationToUse: LanguageDetectorImplementation,
) : AbstractLanguageDetectionAccuracyReport(Language.BURMESE, implementationToUse) {
    @ParameterizedTest
    @CsvFileSource(
        resources = ["/language-testdata/single-words/my.txt"],
    ...
    @ParameterizedTest
    @CsvFileSource(
        resources = ["/language-testdata/word-pairs/my.txt"],
        delimiter = CSV_FILE_DELIMITER,
        encoding = CSV_FILE_ENCODING,
    )
